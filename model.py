...
# Using a generator function to avoid out of memory errors
...
def custommodel(model, dropout):
    ...
    model.add(MaxPool2D(pool_size=(2, 2)))
    model.add(Dropout(dropout))
    #Flatten to 1d and applying fully connected layers
    model.add(Flatten())
    ...
    model.add(Dropout(dropout))
    #no activation because of the regression Purpose of the Network
    model.add(Dense(1, activation = None))
    #using an Adam optimizer and Mean Squared Errors loss function
